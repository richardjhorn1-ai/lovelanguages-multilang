...
export async function generateMetadata({
  ...
  const title = content.page.title.replace('{competitor}', 'Babbel');
  const description = content.page.description.replace('{competitor}', 'Babbel');

  return {
    title,
    description,
    alternates: {
      canonical: `https://www.lovelanguages.io/compare/${lang}/love-languages-vs-babbel/`,
    },
  };
}

export default async function VsBabbelPage({
  params,
}: {
  params: Promise<{ nativeLang: string }>;
}) {
  ...
  const title = content.page.title.replace('{competitor}', 'Babbel');
  const description = content.page.description.replace('{competitor}', 'Babbel');
  const heroTitle = content.page.heroTitle.replace('{competitor}', 'Babbel');
  ...
  const jsonLd = {
    ...
    name: title,
    description,
    url: `https://www.lovelanguages.io/compare/${lang}/love-languages-vs-babbel/`,
    inLanguage: lang,
  };
  ...
  return (
    <>
      ...
      <section className="max-w-5xl mx-auto px-4 py-8">
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 bg-orange-500 rounded-lg flex items-center justify-center text-white text-xl font-bold">B</div>
              <div>
                <h2 className="text-xl font-bold font-header text-gray-900">Babbel</h2>
                <p className="text-sm text-gray-500 font-medium">{content.babbelSubtitle}</p>
              </div>
            </div>
            <ul className="space-y-2 text-gray-600">
              {content.babbelSummary.map((item: { text: string; positive: boolean }, idx: number) => (
                <li key={idx} className="flex items-center gap-2">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="text-center p-4 font-bold text-gray-700">Babbel</th>
                </tr>
              </thead>
              <tbody>
                {content.babbelFeatures.map((feature: { name: string; loveLanguages: { value: string; highlight: boolean }; competitor: { value: string; highlight: boolean } }, i: number) => (
                  <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className={`p-4 text-center ${feature.competitor.highlight ? 'text-orange-600 font-bold' : 'text-gray-600'}`}>
      ...
      <section className="max-w-5xl mx-auto px-4 py-8">
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-gray-50 rounded-2xl p-6">
            <h3 className="text-xl font-bold font-header text-gray-900 mb-4 flex items-center gap-2">
              <span className="text-2xl">{"\uD83D\uDCDA"}</span> {content.chooseBabbelTitle}
            </h3>
            <ul className="space-y-3 text-gray-700">
              {content.chooseBabbel.map((reason: string, idx: number) => (
                <li key={idx} className="flex items-start gap-2">
      ...
      <section className="max-w-3xl mx-auto px-4 py-8">
        <BlogFAQ items={content.babbelFaqs} />
      </section>
      ...
      <section className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex justify-center gap-4 flex-wrap">
          <Link href={`/compare/${lang}/love-languages-vs-duolingo/`} className="px-4 py-2 bg-gray-100 rounded-full text-gray-700 hover:bg-accent/10 hover:text-accent transition-colors">
