...
export async function POST(request: Request) {
  ...
  if (action === 'list') {
    try {
      ...
      return NextResponse.json({ error: 'Failed to fetch summaries' }, { status: 500, headers: corsHeaders });
    }
  }
  ...
  if (action === 'get') {
    ...
    if (!summaryId) {
      return NextResponse.json({ error: 'summaryId required' }, { status: 400, headers: corsHeaders });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: 'Failed to fetch summary' }, { status: 500, headers: corsHeaders });
    }
  }
...
The summary should make them feel seen, not just measured.`;
    ...
    if (saveError) {
      ...
      return NextResponse.json({ error: 'Failed to save journey entry. Please try again.', retryable: true }, { status: 500, headers: corsHeaders });
    }
    ...
    return NextResponse.json({ error: 'Failed to generate summary. Please try again.', retryable: true }, { status: 500, headers: corsHeaders });
  }
}
