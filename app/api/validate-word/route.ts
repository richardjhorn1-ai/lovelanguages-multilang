...
import { buildLightweightSchema } from '@/utils/schema-builders';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const requestWord = body.word || body.polish;
    const { translation, lightweight } = body as ValidateWordRequest;

    if (!requestWord) {
      return NextResponse.json({ error: 'Missing word' }, { status: 400, headers: corsHeaders });
    }
    ...
    if (!apiKey) {
      ...
      if (generateMode) {
        return NextResponse.json({ error: 'AI service required to generate translations' }, { status: 500, headers: corsHeaders });
      }
      return NextResponse.json({
        success: true,
        validated: {
...
Return ONLY the JSON object, no other text.`;
    ...
    return NextResponse.json({ error: 'Failed to validate word. Please try again.' }, { status: 500, headers: corsHeaders });
  }
}
