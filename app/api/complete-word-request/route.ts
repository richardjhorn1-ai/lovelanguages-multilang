...
import { getLanguageName, getLanguageConfig } from '@/constants/language-config';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!requestId) {
      return NextResponse.json({ error: 'Missing requestId' }, { status: 400, headers: corsHeaders });
    }
    ...
    if (requestError || !claimedRequest) {
      ...
      if (!existingRequest) {
        return NextResponse.json({ error: 'Word request not found' }, { status: 404, headers: corsHeaders });
      }
      ...
      if (existingRequest.status === 'completed') {
        return NextResponse.json({ error: 'Already completed' }, { status: 400, headers: corsHeaders });
      }
      if (existingRequest.status === 'in_progress') {
        ...
        if (now - stuckSince > 60_000) {
          ...
          return NextResponse.json({ error: 'Already in progress' }, { status: 400, headers: corsHeaders });
        }
      } else {
        return NextResponse.json({ error: 'Failed to start word request completion' }, { status: 500, headers: corsHeaders });
      }
    } else {
    ...
    if (dictError) {
      ...
      return NextResponse.json({ error: 'Failed to add words to dictionary' }, { status: 500, headers: corsHeaders });
    }
    ...
    // Respond immediately — words are saved, user sees success
    // Background enrichment runs after response in the same invocation
    const responseData = {
      success: true,
    ...
    // In Next.js App Router, we use waitUntil or fire-and-forget.
    ...
    // Fire-and-forget the enrichment
    ...
    return NextResponse.json({ error: 'Failed to add words. Please try again.' }, { status: 500, headers: corsHeaders });
  }
}
