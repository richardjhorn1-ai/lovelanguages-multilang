...
export async function POST(request: Request) {
  ...
  if (!Array.isArray(messages) || !Array.isArray(currentWords)) {
    return NextResponse.json({ error: "Invalid payload. Expecting messages and currentWords arrays." }, { status: 400, headers: corsHeaders });
  }
...
Return as JSON array matching the order above.`;
    ...
    return NextResponse.json({ newWords: sanitizedWords }, { headers: corsHeaders });
  } catch (e: any) {
    console.error("[analyze-history] Error:", e);
    return NextResponse.json({ error: 'Failed to analyze conversation. Please try again.', retryable: true }, { status: 500, headers: corsHeaders });
  }
}
