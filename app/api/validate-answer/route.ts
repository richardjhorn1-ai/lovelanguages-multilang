...
import { enhancedLocalMatch } from '@/utils/local-matcher';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!userAnswer || !correctAnswer) {
      return NextResponse.json({ error: 'userAnswer and correctAnswer are required' }, { status: 400, headers: corsHeaders });
    }
    ...
    if (fastMatch(userAnswer, correctAnswer)) {
      ...
      } as ValidateAnswerResponse, { headers: corsHeaders });
    }
