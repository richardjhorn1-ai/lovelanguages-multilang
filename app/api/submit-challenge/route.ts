...
import { buildBatchValidationSchema } from '@/utils/schema-builders';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { challengeId, answers, timeSpentSeconds } = body;
    ...
    if (!Array.isArray(answers) || answers.length === 0 || answers.length > 100) {
      return NextResponse.json({ error: 'Invalid answers: must be array with 1-100 items' }, { status: 400, headers: corsHeaders });
    }
    ...
    return NextResponse.json({ error: 'Failed to submit challenge. Please try again.' }, { status: 500, headers: corsHeaders });
  }
}
