...
import type { EnhancedCoachContext } from '@/types';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (profile.role !== 'tutor') {
      return NextResponse.json({ error: 'Coach context is only available for tutors' }, { status: 400, headers: corsHeaders });
    }
    ...
    return NextResponse.json({ error: 'Failed to fetch coach context', requestId }, { status: 500, headers: corsHeaders });
  }
}
