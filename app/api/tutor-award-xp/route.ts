  ...
  try {
    ...
    if (targetUserId) {
      ...
      if (callerProfile?.linked_user_id === targetUserId) {
        ...
        return NextResponse.json({ error: 'Not authorized for this action' }, { status: 403, headers: corsHeaders });
      }
    }
    ...
    if (profileError || !tutorProfile) {
      return NextResponse.json({ error: 'Tutor not found' }, { status: 404, headers: corsHeaders });
    }
    ...
    return NextResponse.json({ error: 'Failed to award XP' }, { status: 500, headers: corsHeaders });
  }
}
