...
export async function POST(request: Request) {
  ...
  if (!Array.isArray(transcript) || transcript.length === 0) {
    return NextResponse.json({ error: "Invalid payload. Expecting non-empty transcript array." }, { status: 400, headers: corsHeaders });
  }
...
- Preserve the speaker attribution exactly as given`,
    ...
    return NextResponse.json({ error: 'Failed to process transcript. Please try again.', retryable: true }, { status: 500, headers: corsHeaders });
  }
}
