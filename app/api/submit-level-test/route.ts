...
export async function POST(request: Request) {
  ...
  if (!testId || !answers || !Array.isArray(answers)) {
    return NextResponse.json({ error: 'Missing testId or answers' }, { status: 400, headers: corsHeaders });
  }
  ...
  try {
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: 'Failed to save results' }, { status: 500, headers: corsHeaders });
    }
