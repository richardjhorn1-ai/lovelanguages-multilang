...
const TutorGames: React.FC<TutorGamesProps> = ({ profile }) => {
  ...
  if (loading) {
    return (
      <div className="h-full flex items-center justify-center bg-[#fdfcfd]">
  ...
  if (localGameActive) {
    ...
    if (isGameOver || quickFireTimeUp) {
      ...
      return (
        <div className="h-full flex items-center justify-center p-4 bg-[#fdfcfd]">
          <div className="bg-white p-8 rounded-[2rem] shadow-xl text-center max-w-sm w-full">
            ...
            <div className="text-5xl font-black text-rose-500 mb-4">{percentage}%</div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 px-4 border-2 border-gray-200 rounded-xl font-bold text-gray-600 hover:bg-gray-50"
              >
                Done
              </button>
              <button
                onClick={() => localGameActive === 'quiz' ? startLocalQuiz() : startLocalQuickFire()}
                className="flex-1 py-3 px-4 bg-rose-500 text-white rounded-xl font-bold hover:bg-rose-600"
              >
                Play Again
              </button>
            </div>
    ...
    if (localGameActive === 'quiz') {
      return (
        <div className="h-full flex flex-col p-4 bg-[#fdfcfd]">
          <div className="max-w-md mx-auto w-full">
            {/* Header */}
            <div className="flex items-center justify-between mb-4">
              <button onClick={resetLocalGame} className="p-2 hover:bg-gray-100 rounded-xl">
            ...
            >
              <div className={`relative w-full h-full transition-transform duration-500 transform-style-3d ${localGameFlipped ? 'rotate-y-180' : ''}`}>
                {/* Front */}
                <div className="absolute inset-0 bg-white rounded-[2rem] p-8 flex flex-col items-center justify-center shadow-lg backface-hidden border border-gray-100">
                  <span className="text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-4">Polish</span>
                  <h2 className="text-4xl font-black text-rose-500 text-center">{currentWord?.word}</h2>
                  <p className="text-gray-400 text-sm mt-8">Tap to reveal</p>
                </div>
                {/* Back */}
                <div className="absolute inset-0 bg-rose-500 text-white rounded-[2rem] p-8 flex flex-col items-center justify-center shadow-lg backface-hidden rotate-y-180">
                  ...
                  <div className="mt-8 grid grid-cols-2 gap-3 w-full">
                    <button
                      ...
                      className="py-3 bg-white/20 rounded-xl font-bold text-sm hover:bg-white/30"
                    >
                      ❌ Hard
                    </button>
                    <button
                      onClick={(e) => { e.stopPropagation(); handleLocalQuizResponse(true); }}
                      className="py-3 bg-white rounded-xl font-bold text-sm text-rose-500"
                    >
                      ✓ Got it!
                    </button>
                  </div>
  ...
  return (
    <div className="h-full overflow-y-auto p-4 sm:p-6 bg-[#fdfcfd]">
      <div className="max-w-4xl mx-auto space-y-6">

        {/* Header with Mode Toggle */}
        <div className="text-center">
          <h1 className="text-2xl font-black text-gray-800 mb-3">Play Together</h1>
        ...
        {playMode === 'local' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <button
              ...
              className="group p-6 bg-white rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-md hover:border-rose-200 transition-all text-left disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 bg-rose-50 rounded-xl flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">
                  🎯
                </div>
                <div className="flex-1">
                  <h3 className="font-bold text-gray-800 text-sm">Quiz {partnerName}</h3>
                  <p className="text-xs text-gray-500">Flashcard quiz together</p>
                </div>
              </div>
              <div className="mt-3 flex items-center gap-2 text-rose-500 text-xs font-bold">
            ...
            <button
              ...
              disabled={partnerVocab.length < 4}
              className="group p-6 bg-white rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-md hover:border-amber-200 transition-all text-left disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-bold text-gray-800 text-sm">Quick Fire</h3>
                  <p className="text-xs text-gray-500">60 second challenge</p>
                </div>
              </div>
            ...
            {partnerVocab.length < 4 && (
              <p className="text-xs text-gray-400 text-center col-span-2">
        ...
        {playMode === 'send' && (
          ...
          <button
            ...
            className="group p-6 bg-white rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-md hover:border-rose-200 transition-all text-left"
          >
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 bg-rose-50 rounded-2xl flex items-center justify-center text-3xl group-hover:scale-110 transition-transform">
                🎯
              </div>
              <div className="flex-1">
                <h3 className="font-black text-gray-800 mb-1">Do You Remember?</h3>
                <p className="text-sm text-gray-500">Quiz {partnerName} on their vocabulary</p>
              </div>
            </div>
          ...
          <button
            ...
            className="group p-6 bg-white rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-md hover:border-amber-200 transition-all text-left"
          >
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 bg-amber-50 rounded-2xl flex items-center justify-center text-3xl group-hover:scale-110 transition-transform">
                ⚡
              </div>
              <div className="flex-1">
                <h3 className="font-black text-gray-800 mb-1">Quick Fire</h3>
                <p className="text-sm text-gray-500">Timed vocabulary challenge</p>
              </div>
            </div>
          ...
          <div className="group p-6 bg-gray-50 rounded-[2rem] border border-gray-100 shadow-sm opacity-60 cursor-not-allowed text-left">
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 bg-gray-100 rounded-2xl flex items-center justify-center text-3xl">
                🎤
              </div>
              <div className="flex-1">
                <h3 className="font-black text-gray-600 mb-1">Whisper Game</h3>
                <p className="text-sm text-gray-400">Pronunciation practice</p>
              </div>
            </div>
          ...
          <button
            ...
            className="group p-6 bg-white rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-md hover:border-teal-200 transition-all text-left"
          >
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 bg-teal-50 rounded-2xl flex items-center justify-center text-3xl group-hover:scale-110 transition-transform">
                🎁
              </div>
              <div className="flex-1">
                <h3 className="font-black text-gray-800 mb-1">Gift Words</h3>
          ...
          <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-6">
            ...
            <div className="space-y-3">
              {pendingChallenges.slice(0, 3).map(challenge => (
                <div key={challenge.id} className="flex items-center gap-3 p-3 bg-amber-50 rounded-xl">
        ...
        {playMode === 'send' && pendingRequests.length > 0 && (
          <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-6">
            <h3 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
              <ICONS.Heart className="w-4 h-4 text-rose-400" />
              Word Gifts Sent
            </h3>
            <div className="space-y-3">
              {pendingRequests.slice(0, 3).map(request => (
                <div key={request.id} className="flex items-center gap-3 p-3 bg-teal-50 rounded-xl">
          ...
          <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-6">
            ...
            <div className="space-y-3">
              {completedChallenges.slice(0, 5).map(challenge => (
                <div key={challenge.id} className="flex items-center gap-3 p-3 bg-green-50 rounded-xl">
