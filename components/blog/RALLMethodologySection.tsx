...
interface Props {
  nativeLang: string;
  seed: string; // article slug -- drives deterministic rotation
}
...
const SECTION_COPY: Record<string, { heading: string; subtitle: string; seeAll: string }> = {
  ...
  ro: { heading: '\u00cenva\u021ba\u021bi Limba Partenerului \u00cempreun\u0103', subtitle: 'Metodologia noastr\u0103 RALL transform\u0103 rela\u021bia voastr\u0103 \u00eentr-o superputere lingvistic\u0103.', seeAll: 'Vezi Toate Articolele de Metodologie' },
...
export default function RALLMethodologySection({ nativeLang, seed }: Props) {
  ...
  return (
    <section className="max-w-3xl mx-auto px-4 pb-12">
      <div className="rounded-2xl overflow-hidden bg-gradient-to-br from-violet-600 to-pink-500 p-[1px]">
        <div className="bg-white rounded-2xl p-6 md:p-8">
          {/* Header */}
          <div className="text-center mb-6">
            <span className="text-3xl mb-2 block">{'\u{1F491}'}</span>
            <h3 className="text-xl md:text-2xl font-bold font-header text-gray-900 mb-2">
              {copy.heading}
            </h3>
          ...
          <div className="space-y-3 mb-6">
            {articles.map((article) => (
              <a
                ...
                className="flex items-start gap-3 p-3 rounded-xl border border-gray-100 hover:border-violet-200 hover:bg-violet-50/50 transition-all group"
              >
                ...
                <svg className="w-4 h-4 text-gray-400 group-hover:text-violet-500 flex-shrink-0 mt-1 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="text-center">
            <a
              href={indexUrl}
              className="inline-flex items-center gap-1.5 text-sm font-bold text-violet-600 hover:text-violet-800 transition-colors"
            >
              {copy.seeAll}
