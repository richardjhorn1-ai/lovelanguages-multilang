...
2. Enable Associated Domains on Apple Developer App ID `com.lovelanguages.app`, regenerate provisioning, and verify Universal Links on device.
3. Upload a new iPhone-only build that includes the Sign in with Apple and Apple-IAP-only fixes, then use that build for TestFlight and App Review response.
