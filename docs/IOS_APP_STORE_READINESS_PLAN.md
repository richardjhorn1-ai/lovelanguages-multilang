...
| Native/Xcode | Real iOS target audited | native/Xcode verified | engineering | A checked-in `ios/` project now exists locally, the target builds with `xcodebuild` for device and simulator with code signing disabled, and the tracked project now carries `DEVELOPMENT_TEAM = H2UMXHNSK4`. A real Release archive reaches Apple provisioning and currently stops at profile generation. | Keep the tracked iOS project committed and use it for all native release work. |
...
| App Store Connect / RevenueCat / Supabase | Current uploaded build matches the iPhone-first launch scope | App Store Connect / RevenueCat / Supabase verified | product / engineering | TestFlight build `1.0 (2)` still reports device family `iPhone, iPad`, and the rejected App Review used `iPad Air 11-inch (M3)`. The uploaded build does not yet reflect the local iPhone-only target. A March 8, 2026 archive attempt against the current tracked project now reaches provisioning but fails because Apple cannot generate the required profile while the team has no registered devices available to Xcode. | Connect a physical iPhone or manually register device IDs in Apple Developer, refresh provisioning, then upload the next iPhone-only build for TestFlight and re-review. |
...
2. Refresh provisioning/signing now that Associated Domains is enabled and `DEVELOPMENT_TEAM = H2UMXHNSK4` is set in the tracked project. Connect a physical iPhone to this Mac or manually register device IDs in Apple Developer so Xcode can mint the required profile.
3. Upload a new iPhone-only build that includes the Sign in with Apple, universal-link, and Apple-IAP-only fixes, then use that build for TestFlight and App Review response.
4. Complete metadata for all 6 App Store subscription products and refresh the review notes/screenshots for the new build.
5. Run physical-device tests for:
   - auth callback
   - password reset
   - invite open
   - Apple IAP purchase
   - restore purchases
6. Verify at least one real sandbox RevenueCat purchase event (`INITIAL_PURCHASE` or `RENEWAL`) mutates backend state on the new build and logs cleanly in production.
