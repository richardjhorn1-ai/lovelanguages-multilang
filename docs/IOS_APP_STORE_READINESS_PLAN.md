...
- The tracked project now carries `DEVELOPMENT_TEAM = H2UMXHNSK4`, and a physical team device was registered with Apple Developer so Xcode can mint signing assets.
- A signed Release archive and App Store export were produced locally on March 8, 2026:
  - archive: `/tmp/LoveLanguages.xcarchive`
  - export: `/tmp/LoveLanguagesExport/App.ipa`
...
- An app-level `PrivacyInfo.xcprivacy` now exists in the tracked target and is bundled as a resource.
- Still not verified from this session:
  - privacy manifest parity against actual SDK and data-use disclosures
...
- Live RevenueCat project `Love Languages` was audited. The App Store app uses bundle ID `com.lovelanguages.app`, all 6 expected products exist, the default offering contains all 6 packages, live entitlements are currently named `Standard` and `Unlimited`, and the production webhook is now configured and test-verified against `https://www.lovelanguages.io/api/webhooks/revenuecat/`.
...
- Live Apple Developer was audited. App ID `com.lovelanguages.app` now has In-App Purchase, Sign in with Apple, Push Notifications, and Associated Domains enabled. Service ID `com.lovelanguages.app.auth` was configured in-session with `www.lovelanguages.io` and `auth.lovelanguages.io` plus the return URL `https://auth.lovelanguages.io/auth/v1/callback`, although Apple’s post-save UI still did not provide a clean readback of the saved values.
...
| Native/Xcode | Real iOS target audited | native/Xcode verified | engineering | A checked-in `ios/` project now exists locally, the target builds with `xcodebuild` for device and simulator, the tracked project carries `DEVELOPMENT_TEAM = H2UMXHNSK4`, a physical team device was registered with Apple Developer, and a signed Release archive plus App Store export were produced locally on March 8, 2026 (`/tmp/LoveLanguages.xcarchive`, `/tmp/LoveLanguagesExport/App.ipa`). | Keep the tracked iOS project committed and use it for all native release work. |
...
| Native/Xcode | Associated Domains configured for Universal Links | native/Xcode verified | engineering | `App.entitlements` now includes `applinks:www.lovelanguages.io`, Apple Developer has Associated Domains enabled for `com.lovelanguages.app`, and `https://www.lovelanguages.io/.well-known/apple-app-site-association` now returns the expected JSON for `H2UMXHNSK4.com.lovelanguages.app`. The root-path fallback `/apple-app-site-association` still 308-redirects, so the launch contract should rely on the `.well-known` endpoint. | Verify actual Universal Link takeover on device after refreshed provisioning. |
...
| Native/Xcode | Privacy manifest and any required capabilities are correct | still unknown | engineering | The app target now includes a checked-in `PrivacyInfo.xcprivacy` bundled in the tracked Xcode target, but parity against actual SDK/data usage and any required native capability manifests is not yet fully proven. | Cross-check the manifest against shipped SDK behavior and update if Apple-required reasons or data categories are missing. |
...
| App Store Connect / RevenueCat / Supabase | RevenueCat webhook is configured and end-to-end delivery is verified | App Store Connect / RevenueCat / Supabase verified | engineering | On March 8, 2026 the RevenueCat webhook was configured to use `https://www.lovelanguages.io/api/webhooks/revenuecat/` with the shared bearer token. The malformed production `REVENUECAT_WEBHOOK_SECRET` was corrected in Vercel, `main` was redeployed on production build `lovelanguages-multilang-1hn5zswm5-richardjhorn1-6035s-projects.vercel.app`, direct POSTs now return `200`, and RevenueCat’s own `TEST` event now returns `200` and is logged by the webhook handler. | During device QA, verify at least one real sandbox `INITIAL_PURCHASE` or `RENEWAL` event mutates backend subscription state as expected. |
| Repo | Clean Vercel production rebuilds install nested blog dependencies deterministically | repo verified | engineering | A March 8, 2026 Vercel production redeploy of `main` initially failed with `npm run build` exit `127` because `blog` dependencies were not installed on a clean cloud build, causing `astro: command not found`. The repo now declares `installCommand: npm ci && npm --prefix blog ci` in `vercel.json`, the full root build passes locally when run with the production-style env/network assumptions, and the follow-up production deployment `lovelanguages-multilang-1hn5zswm5-richardjhorn1-6035s-projects.vercel.app` completed successfully. | Keep the install command in place and watch future Vercel production builds for regressions. |
| App Store Connect / RevenueCat / Supabase | Current uploaded build matches the iPhone-first launch scope | App Store Connect / RevenueCat / Supabase verified | product / engineering | TestFlight build `1.0 (2)` still reports device family `iPhone, iPad`, and the rejected App Review used `iPad Air 11-inch (M3)`. The uploaded build does not yet reflect the current local iPhone-only target, but a fresh signed export now exists locally as `1.0 (3)` at `/tmp/LoveLanguagesExport/App.ipa`. | Upload the new iPhone-only build to App Store Connect, then use that build for TestFlight and re-review. |
| App Store Connect / RevenueCat / Supabase | Apple portal capability state matches the native routing and billing contract | App Store Connect / RevenueCat / Supabase verified | engineering | Apple Developer App ID `com.lovelanguages.app` now has In-App Purchase, Sign in with Apple, Push Notifications, and Associated Domains enabled, and the live AASA contract is hosted at `https://www.lovelanguages.io/.well-known/apple-app-site-association`. | Refresh provisioning/signing assets and verify Universal Links on device. |
| App Store Connect / RevenueCat / Supabase | Apple service ID callback config matches Supabase auth expectations | App Store Connect / RevenueCat / Supabase verified | engineering | On March 8, 2026, Service ID `com.lovelanguages.app.auth` was reconfigured in Apple Developer with website domains `www.lovelanguages.io` and `auth.lovelanguages.io` plus the return URL `https://auth.lovelanguages.io/auth/v1/callback`, and Apple accepted the save against primary App ID `H2UMXHNSK4.com.lovelanguages.app`. Apple’s post-save modal remains hard to read back, so the final proof still has to come from device auth QA. | Prove the config with a real Sign in with Apple round-trip on device and keep the values aligned with Supabase. |
...
| App Store Connect / RevenueCat / Supabase | App Privacy answers are published and consistent with the shipped app | App Store Connect / RevenueCat / Supabase verified | product / engineering | App Privacy is published and lists tracking plus linked/not-linked data categories, and the repo now includes an app-level `PrivacyInfo.xcprivacy`, but parity between those disclosures and actual shipped SDK/data usage is not yet proven. | Cross-check actual SDK/data usage, update the manifest if needed, and reconcile any App Privacy answer drift before resubmission. |
...
- Refresh provisioning/signing after the Apple Developer capability change.
- Device-test Universal Link takeover for invite opens, password reset, and auth callbacks.
- Keep the live contract on `/.well-known/apple-app-site-association`; do not rely on the root fallback path while it still redirects.

### Phase 3: Native Submission Surface Verification

Status: in progress. Repo/native config and Apple portal state are mostly aligned, but signed archive generation is still blocked on provisioning refresh for team `H2UMXHNSK4`.
...
2. Upload the new iPhone-only signed export (`/tmp/LoveLanguagesExport/App.ipa`) that includes the Sign in with Apple, universal-link, and Apple-IAP-only fixes, then use that build for TestFlight and App Review response.
3. Complete metadata for all 6 App Store subscription products and refresh the review notes/screenshots for the new build.
4. Run physical-device tests for:
   ...
   - Apple IAP purchase
   - restore purchases
5. Verify at least one real sandbox RevenueCat purchase event (`INITIAL_PURCHASE` or `RENEWAL`) mutates backend state on the new build and logs cleanly in production.
6. Harden the native purchase pending-confirmation UX if delayed webhook confirmation still feels fragile on device.
