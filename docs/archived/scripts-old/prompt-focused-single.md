...
## Design Direction: "Focused Single-Screen"

Everything that matters is above the fold. One screen, one action. Maximum clarity.

Think: Duolingo.com landing page level of simplicity. Or Linear.app. Or Notion's "Get started" page.

## Layout

### Above the fold (`min-h-screen` with flex centering):

**Desktop (`md:flex-row`):**
- LEFT side: Headline ("Learn their language" or similar) + subtitle + IllustrationPlaceholder
- RIGHT side: Compact language picker (popular 8 languages as a 4x2 grid of flag buttons) + CompactLoginForm
...
Header: Logo on left, Learn/Teach toggle on right. Minimal.

### Below the fold (scroll down for more):
- 3 feature highlights in a grid
- HeroFooter

The above-fold section should feel complete on its own — the user shouldn't need to scroll to understand or take action. Below the fold is bonus reassurance.
...
Return a single complete React component file (FocusedSingle.tsx). Default export. No other files.
