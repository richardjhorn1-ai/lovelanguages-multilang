You are a designer who writes production React code. I need a homepage for Love Languages.

Read everything below carefully — it tells you what the product is, who it's for, and what tools you have. Then design a homepage. I'm not going to tell you what the layout should be. That's your job. Surprise me.

## Your creative brief

**Concept name:** "Concept B"
**Filename:** ConceptB.tsx

**The feeling:** This page should make you want to DO something, not read something. The best pitch for a product is the product itself. If someone can try it — even a taste — before signing up, that's more convincing than any headline.

**One constraint on approach:** This concept should prioritize INTERACTION over EXPLANATION. Get the visitor clicking, choosing, playing within seconds. The GameShowcase component lets people try real flashcard games without an account. Use that (or the language selection flow, or both). Minimize the distance between "landing on the page" and "doing something fun."

**Inspiration (for vibe, not layout):** When a friend hands you their phone and says "just try this game." The way a good playground has no instructions — kids just start playing. Stripe's interactive API demos where you're coding before you realize you're being sold to.

**What good looks like:** Someone opens this page on their phone during lunch, picks Spanish, plays 3 flashcards, and signs up before they finish their sandwich.

## What is Love Languages?

Love Languages is a language learning app built for couples. One partner is the native speaker (tutor), the other is learning (student). It's not Duolingo — there are no leaderboards, no streaks against strangers, no generic "order coffee" phrases. Everything is built around YOUR relationship.

The founder's story: "It started because I kept forgetting. Misia would teach me a word in Polish… By morning it was gone. We tried the apps everyone recommends. But it was always just me and a screen. Misia couldn't see where I was stuck… So I built something to catch the words she taught me."

**Core insight:** Your partner is the best language teacher you'll ever have. They know what words matter. They know when you're faking understanding. They want you to succeed more than any app ever could.

## Who uses it

**Two roles, one couple:**

**Students (learners):**
- Want to understand when their partner's family is talking
- Tired of nodding along and pretending they understand
- Want to make their partner's mom laugh — in her language
- Fear of judgment keeps them from trying
- They don't need to be fluent, they need to belong

**Tutors (native speakers):**
- Have a whole side of themselves their partner has never met
- Frustrated watching meaning die in translation
- Want to share their language — their jokes, their childhood, their world
- Want to SEE their partner's progress, guide it, celebrate it

**The emotional core:** Learning your partner's language isn't about vocabulary. It's about saying "your world matters to me enough that I'll do the hard thing."

## What makes it different

| Traditional apps | Love Languages |
|---|---|
| Learn alone against strangers | Learn together with your partner |
| Generic curriculum (airports, restaurants) | Partner-guided vocabulary from YOUR life |
| Competitive (streaks, leaderboards) | Cooperative (shared Love Log, word gifts) |
| Same experience for everyone | Two distinct roles: student and tutor |
| Text-heavy lessons | AI conversation practice + 6 game modes |
| Progress is private | Every milestone is shared and celebrated |

**The one-liner:** Traditional apps teach you to order coffee. Love Languages teaches you to make your partner's family laugh.

## Key features

**For students:** AI conversation practice, 6 game modes (flashcards, multiple choice, type it, quick fire, AI challenge, conversation), Listen Mode (record real conversations, capture missed words), Love Log (auto-saved vocabulary from real life), voice chat, weakest-word tracking

**For tutors:** Agentic coach (data-driven teaching guidance), word gifts (send curated vocab like "words for meeting my grandmother"), progress tracking, partner challenges, AI teaching assist, shared activity feed

**Pricing:** Free trial (no card) → $19/mo standard → $39/mo unlimited. 18 supported languages, any combination of native/target.

## The current homepage problems

Our current homepage is a 1,300-line monster component with 18 useState hooks. Here's what's wrong:

- **Two separate codepaths** for mobile and desktop — 600+ duplicate lines
- **Horizontal carousel** on mobile with snap-scrolling, drag-to-expand bottom sheet, pagination dots — way overengineered
- **Text-heavy marketing sections** — 5+ scrollable cards with long paragraphs nobody reads
- **Buried signup** — you have to scroll through marketing walls to find the login form
- **Too many states** — carousel position, drag offset, sheet expansion, step transitions, intersection observers
- **Separate mobile bottom sheet** with touch handlers, snap points, 80px drag thresholds
- **Marketing copy that sounds like AI wrote it** — long, generic, forgettable

We want to burn it down and start fresh. Something simple, warm, and immediate.

## Brand personality

- **Warm and romantic** — not corporate, not startup-bro
- **Cheeky but not childish** — "Learn the sweet nothings, the spicy somethings, and everything in between"
- **Emotionally honest** — acknowledges that language barriers in relationships are painful
- **Supportive** — low-judgment, encouraging, celebrating small wins
- **NOT:** generic, polished-startup, productivity-tool, gamified, competitive

## 18 supported languages

English, Spanish, French, German, Italian, Portuguese, Dutch, Polish, Russian, Ukrainian, Swedish, Norwegian, Danish, Czech, Greek, Hungarian, Turkish, Romanian — any can be native or target.


## Building Blocks

You have these tools available. Use whichever ones serve your design — you don't have to use all of them.

### Required (MUST use)

1. **useHeroLogic hook** — call `const logic = useHeroLogic()` for all state and handlers
2. **CompactLoginForm** — render `<CompactLoginForm logic={logic} />` for auth (Google, Apple, email, magic link)

### Available components (import if useful, don't recreate)

| Component | Import | What it does |
|-----------|--------|-------------|
| NativeLanguagePill | `from '../hero/NativeLanguagePill'` | Dropdown pill for selecting native language |
| LanguageGrid | `from '../hero/LanguageGrid'` | Grid of target languages with flags and native names |
| GameShowcase | `{ GameShowcase } from '../hero/GameShowcase'` | Interactive demo of the flashcard/game modes |
| HeroFAQ | `{ HeroFAQ } from '../hero/HeroBottomSections'` | FAQ accordion |
| HeroFooter | `{ HeroFooter } from '../hero/HeroBottomSections'` | Page footer |
| LOGO_PATH, LOGO_DETAIL_PATHS | `from '../hero/Section'` | SVG path data for logo (bird with hearts) |
| ICONS | `from '../../constants'` | Icon components: Heart, Sparkles, ChevronLeft, ChevronRight, etc. |
| LANGUAGE_CONFIGS | `from '../../constants/language-config'` | `{ code, flag, nativeName, name }` for all 18 languages |
| BRAND | `from '../hero/heroConstants'` | Color constants: primary, teal, light, tealLight, shadow |

### Logo

**See the attached logo.svg** — it's a bird with hearts. In code:
```tsx
...
```
Color it with `logic.accentColor` — coral (#FF4761) for students, teal (#14b8a6) for tutors.

### Key state from the hook

```
logic.isStudent          → boolean (true = student/coral, false = tutor/teal)
logic.selectedRole       → 'student' | 'tutor'
logic.setSelectedRole    → switch between roles (changes all colors)
logic.accentColor        → current accent color string
logic.bgColor            → current background color string
logic.accentShadow       → box-shadow color string
logic.nativeLanguage     → auto-detected native language code or null
logic.selectedTargetLanguage → selected target language code or null
logic.handleTargetSelect → (code: string) => Promise<void>
logic.handleNativeSelect → (code: string) => Promise<void>
logic.t                  → i18n translation function
```

### Visuals

We don't have illustrations yet — but DON'T just slap in grey placeholder boxes. Get creative. Use CSS/Tailwind/SVG to build something real: abstract shapes, animated gradients, emoji compositions, decorative patterns, the logo as a hero element, flag arrangements, anything that feels designed and intentional. Think of it as a constraint that forces creativity, not a limitation. The visual areas should feel like they belong, not like something's missing.


## Technical Constraints

- **Single file** — one default-exported React functional component
- **React + Tailwind CSS** — mobile-first with `md:` breakpoint at 768px
- **One component for all screens** — responsive, NOT separate mobile/desktop layouts
- **Fonts:** Quicksand for headers (`font-header` class), Outfit for body (default)
- **200-400 lines** of clean, production-quality code
- **CSS-only animations** — no animation libraries, no new npm dependencies
- **i18n:** Use `logic.t('key')` for existing translation keys. For new copy, hardcode English with `// TODO: i18n` comment
- **No AI slop copy** — no "revolutionary", "cutting-edge", "transform your journey". Write like a human who gives a shit.

### Imports

```tsx
import { useHeroLogic } from '../../hooks/useHeroLogic';
import CompactLoginForm from './shared/CompactLoginForm';
// + whatever optional components you choose to use
```

## Source code reference

### useHeroLogic hook (the interface you MUST use):
```typescript
/**
...
### CompactLoginForm (you MUST render this somewhere):
```typescript
/**
...
Return a single complete React component file (ConceptB.tsx). Default export. Nothing else.
