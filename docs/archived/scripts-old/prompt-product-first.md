...
The product IS the pitch. Don't describe what the app does — SHOW it.

Key principles:
- Lead with an interactive demo, not a marketing headline
- Minimize text — let the experience speak
- Simple vertical flow
- Signup appears naturally after the user has engaged

## Layout (top to bottom)

1. **Minimal header**: Just the logo centered, clean
2. **Headline**: "Try it. Right now." (or similar — short, punchy, inviting)
3. **Quick language picker**: Show 6-8 popular languages as flag buttons in a horizontal flex row. Use LANGUAGE_CONFIGS for flag + nativeName. Clicking one calls `logic.handleTargetSelect(code)`. Auto-detect native from `logic.nativeLanguage`.
4. **GameShowcase**: The centerpiece. Full width, prominent. Only show when `logic.selectedTargetLanguage` is set. Use the existing GameShowcase component.
5. **Transition to signup**: A simple line like "Like it? Keep going." then the Learn/Teach role toggle + CompactLoginForm
6. **Footer**: HeroFooter at the bottom

The feel should be: curiosity → engagement → conversion. No walls of text between the user and the product.
...
Return a single complete React component file (ProductFirst.tsx). Default export. No other files.
