You are a senior frontend designer-developer. Your task is to generate 2-3 production-quality React + Tailwind CSS homepage redesign concepts for the "Love Languages" app — a language learning app for couples.

## Design Direction

Follow Duolingo's DESIGN PRINCIPLES (NOT their gamification):
- **Illustration-first**: Visual areas tell the story, not paragraphs of text. Use IllustrationPlaceholder divs with descriptive labels for where illustrations would go.
- **Single clear action per screen**: Don't overwhelm. One thing to do at a time.
- **Generous whitespace**: Breathing room. Nothing feels crowded.
- **Simple vertical scroll**: No carousels, no horizontal scrolling, no split-screen layouts, no bottom sheets.
- **Product-as-pitch**: Show the product, don't just describe it with text.

## Technical Requirements

Each concept MUST:
1. `import { useHeroLogic } from '../../hooks/useHeroLogic'` and call it as: `const logic = useHeroLogic()`
2. `import CompactLoginForm from './shared/CompactLoginForm'` and render it: `<CompactLoginForm logic={logic} />`
3. Be a single default-exported React functional component
4. Use Tailwind CSS for all styling (mobile-first with `md:` breakpoint at 768px)
5. Be responsive — same component works for mobile and desktop (NO separate mobile layout)
6. Use the design system fonts: Quicksand for headers (`font-header` class), Outfit for body (default)
7. Use the brand colors from the logic hook (`logic.accentColor`, `logic.bgColor`, etc.)
8. The `logic.isStudent` boolean toggles between coral pink and teal green accent
9. Include a Learn/Teach role toggle using `logic.selectedRole` and `logic.setSelectedRole`
10. Include language selection somewhere using `logic.nativeLanguage`, `logic.selectedTargetLanguage`, `logic.handleTargetSelect`, etc.

## What to Use for Illustrations

Since we don't have illustrations yet, create placeholder areas like:
```tsx
<div
...
### Concept A: "Clean Vertical" (filename: CleanVertical.tsx)
- Simple vertical scroll page
- Sticky header with logo + Learn/Teach toggle
- Big hero headline + illustration placeholder + CTA
- Language selection section (use LanguageGrid)
- 3 feature highlights (icon + single line of text, in a grid)
- Inline CompactLoginForm
- HeroFAQ + HeroFooter at bottom

### Concept B: "Product-First" (filename: ProductFirst.tsx)
- Lead with an interactive demo — use GameShowcase component
- Minimal header (just logo)
- "Try it. Right now." headline
- Quick language picker (show popular languages as flag buttons)
- GameShowcase takes up most of the viewport
- CompactLoginForm appears below the demo
- HeroFooter at bottom

### Concept C: "Focused Single-Screen" (filename: FocusedSingle.tsx)
- Everything important above the fold (min-h-screen with flex centering)
- Desktop: side-by-side layout (md:flex-row) — left = headline + illustration, right = language picker + CompactLoginForm
- Mobile: stacked vertically
- Below the fold: 3 feature highlights + HeroFooter
- Maximum whitespace, maximum clarity
...
These components already exist and can be imported. Do NOT recreate them.

### NativeLanguagePill
Import: `import NativeLanguagePill from '../hero/NativeLanguagePill'`
- Dropdown pill showing current native language with flag
- Props: `{ nativeLanguage: string | null, isStudent: boolean, onSelect: (code: string) => Promise<void> }`

### LanguageGrid
Import: `import LanguageGrid from '../hero/LanguageGrid'`
- Grid of target language options with flag emojis and native names
- Props: `{ onSelect: (code: string) => Promise<void>, selectedCode: string | null, excludeCode: string | null, isStudent: boolean, title: string, subtitle: string }`

### GameShowcase
Import: `import { GameShowcase } from '../hero/GameShowcase'`
- Interactive demo showing flashcard/game modes
- Props: `{ isStudent: boolean, accentColor: string, sectionIndex: number, isMobile?: boolean, targetLanguage: string | null, nativeLanguage: string | null }`

### HeroFAQ, HeroFooter
Import: `import { HeroFAQ, HeroFooter } from '../hero/HeroBottomSections'`
- FAQ accordion and page footer
- Props: `{ isStudent: boolean, sectionIndex: number, isVisible: boolean }`

### LOGO_PATH, LOGO_DETAIL_PATHS
Import: `import { LOGO_PATH, LOGO_DETAIL_PATHS } from '../hero/Section'`
- SVG path data for the Love Languages logo (bird with hearts)
- Use in an SVG: `<svg viewBox="0 0 600.000000 600.000000"><g transform="translate(0,600) scale(0.1,-0.1)"><path d={LOGO_PATH} />{LOGO_DETAIL_PATHS.map((d, i) => <path key={i} d={d} />)}</g></svg>`

### ICONS
Import: `import { ICONS } from '../../constants'`
- Icon components: Heart, Sparkles, ChevronLeft, ChevronRight, etc.
- Usage: `<ICONS.Heart className="w-5 h-5" />`

### LANGUAGE_CONFIGS
Import: `import { LANGUAGE_CONFIGS } from '../../constants/language-config'`
- Object mapping language codes to `{ code, flag, nativeName, name }`
- Example: `LANGUAGE_CONFIGS['es'] -> { code: 'es', flag: 'spanish flag', nativeName: 'Espanol', name: 'Spanish' }`

### BRAND
Import: `import { BRAND } from '../hero/heroConstants'`
- primary: '#FF4761' (coral, student accent)
- teal: '#14b8a6' (tutor accent)
- light: '#FFF0F3' (pink background)
- tealLight: '#ccfbf1' (teal background)
...
Return EXACTLY 3 code blocks, each wrapped in a filename marker:

--- CleanVertical.tsx ---
(full component code here in a tsx code block)
...
Each component should be 200-400 lines of clean, production-quality React + Tailwind code. Use real i18n keys where they exist (logic.t('hero.login.signIn')), and hardcoded English strings with a comment "// TODO: i18n" where new copy is needed.

DO NOT:
- Create separate mobile/desktop components
- Use any animation libraries (CSS only)
- Add new npm dependencies
- Create more than 3 concepts
- Include any copy that sounds like marketing AI slop
